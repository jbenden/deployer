...
@pytest.mark.skipif(IS_WINDOWS, reason='Irrelevant on non-unix')
def test_plugin_command_with_register_on_unix(caplog, reactor):  # noqa: no-cover
    stream = StringIO('''
    - name: test1
      command: echo Hello
      register: test1
...
@pytest.mark.skipif(IS_WINDOWS, reason='Irrelevant on non-unix')
def test_plugin_command_with_register_result_on_unix(caplog, reactor):  # noqa: no-cover
    stream = StringIO('''
    - name: test1
      command: echo Hello
      register: test1
...
@pytest.mark.skipif(not IS_WINDOWS, reason='Irrelevant on non-Windows')
def test_plugin_command_with_register_on_win(caplog, reactor):  # noqa: no-cover
    stream = StringIO('''
    - name: test1
      command: \'\Windows\System32\cmd.exe /c echo Hello world\'
      register: test1
...
@pytest.mark.skipif(not IS_WINDOWS, reason='Irrelevant on non-Windows')
def test_plugin_command_with_register_result_on_win(caplog, reactor):  # noqa: no-cover
    stream = StringIO('''
    - name: test1
      command: \'\Windows\System32\cmd.exe /c echo Hello world\'
      register: test1
